...
* xref:climatology::index.adoc[Climatology_pi] Data for default examples and longer voyages. 30 years of NOAA wind, wave & Current data, averaged by month.
...
The quest for _perfect_ polars. There are many factors involved.

* xref:polar::index.adoc[Polar_pi] Create Polar files from nmea0183 data.
* xref:vdr::index.adoc[VDR_pi] Record OpenCPN nmea0183 data stream
...
** Polar sample files (700) provided. Accuracy on your boat may vary.
** Actual performance measured in specific sea conditions (without current) recorded by xref:vdr::index.adoc[VDR_pi] and converted by xref:polar::index.adoc[Polar_pi].
...
* xref:ocpn_odraw::odraw.adoc [ODraw_pi] Create boundaries for routing avoidance: dangerous areas, sea, land, storm or hurricane.
...
Weather Routing is a deep plugin which can use six other plugins concurrently, so stick to basics when starting out.
...
xref:opencpn-plugins:misc:plugin-install.adoc[Install Plugins] and xref:opencpn-plugins:misc:plugin-install.adoc#_plugin_enabledisable[Enable] the weather_routing plugin from within OpenCPN using _Options > Plugins_

...
* Your boat's xref:<<boat_performance_data_polar_data>>[Boat Performance Data - Polar Data] or xref:<<boat_performance_data_outside_sources>>[Outside Polar File Sources]. Accuracy in given conditions is significant. 
...
* xref:ocpn_odraw::odraw.adoc [ODraw_pi] to create "boundarys" for routing avoidance of sea and land areas.
...
Example: Routing from Cuba to Puerto Rico through Hurricane Irma in avoidance mode.(Don't ever try to do this!) Showing Grib_pi “Weather Data” (Right Click on Chart) and *Weather_routing View > Report & View > Plot*.  See the Irma Hurricane Examples page down below Item #22, after you have finished this.
...
* *Configuration* The plugin comes installed ready with a number of configurations to _Compute_ successfully.
...
. The goal here is to *Complete* your first routing successfully.
...
xref:climatology::index.adoc[Climatology_pi] should be installed and enabled as directed.
...
The plugin is packaged wit 4 configurations using *Climatology Wind Data* that should *Compute* out of the box. The 4 Weather Routing Configurations in the *Weather-Routing-Setup* with require installation and Enabling *Climatology_pi* and selecting Climatology dropdown menu  _Most Likely_ in the WR Configuration. +

=== 9. WR use requires realistic goals & sound judgment

You should _Configure_ weather_routing to match your _sound judgment_ and _realistic goals_. You are in control, use your own judgement when you review the results. Weather_Routing_pi is just a tool in your hands, you are in control.
...
If the plugin is installed and enabled,  with default settings unchanged (or “Reset all Advanced” is used), the plugin should create an optimized weather routing with just a few specific additional settings and “computation”.

* Complete xref:#_1_setup_grib_pi[Setup Grib_pi Data] and xref:#_2_setup_climatology_pi[Setup
...
. Review the xref:irma_examples.html[Irma Hurricane Examples] page and download and install the example files.
...
xref:polar_conversion.adoc[Polar File Conversion] Examples

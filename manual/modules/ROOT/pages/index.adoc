...
* xref:#_boat_performance_data_-_polar_data[Boat
...
* xref:#_settings.html[Configuration Defaults] Options and Constraints you may need for reference.
...
link:https://opencpn.org/wiki/dokuwiki/doku.php?id=opencpn:opencpn_user_manual:plugins:install_and_enable#enabling_a_plugin[Enable] the plug-in in OpenCPN.
...
xref:opencpn-plugins:chart_downloader_tab:chart_downloader_tab.adocr[Chart downloader]
...
* Complete xref:#_setup_grib_pi_data[Setup Grib_pi Data] and xref:#_setup_climatology_pi_data[Setup
...
as xref:#_weather_routing_setup[Weather_Routing_Setup]* above.
...
xref:#_weather_routing_setup[Weather_Routing_Setup] above.
...
download the xref:#_weather_routing_setup[Weather_Routing_Setup] above, install it again and start over.

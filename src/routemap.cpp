                                        ...
                                        double degree_step, BoatSpeed &boat)
{
  ...
  for(double W = 0; W < DEGREES; W += degree_step) {
    ...
    double dist = .1; //VBG / 3600000.0 * dt.GetMilliseconds().ToDouble();
...
/* Take a route which overlaps with itself and convert to a list
   of proper routes which no longer have overlaps */
      ...
      if(dir) {
        ...
        for(RouteList::iterator it = inner.begin(); it != inner.end(); ++it) {        
          if(direction != (*it)->direction) {
            ...
            /* dont need routes with same dir */
          }
        }
...
Route *Merge(Route *route1, Route *route2)
{
  ...
  Position *q = p->next;
  while(q != route1->points) {
    Position *r = route2->points;
    Position *s = r->next;
    while(s != route2->points) {
      ...
      if(dir) {
        ...
        if(route1->children.size() || route2->children.size()) {
          printf("cannot merge routes with children\n");
          exit(3);
        }
  ...
  /* no intersection found, if one route is
  ...
  do {
    ...
    p = p->next;
  } while(p != route2->points);

  /* routes close enough to fail initial rectangle test but no
     actual intersection or overlap occurs */
